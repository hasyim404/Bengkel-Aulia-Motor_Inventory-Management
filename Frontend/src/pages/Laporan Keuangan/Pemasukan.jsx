...
const Pemasukan = () => {
  ...
  const records = allPemasukan.slice(firstIndex, lastIndex);
  const npage = Math.ceil(allPemasukan.length / recordsPerPage);
  ...
  const getPemasukan = async () => {
    ...
    setAllPemasukan(response.data.data);
    setLoading(false);
  };
  ...
  const addPemasukan = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        title: "Tambah Data Pemasukan Berhasil!",
      ...
      console.log(error);
      Swal.fire({
        title: "Gagal tambah data pemasukan!",
  ...
  const deletePemasukan = async (id) => {
    try {
      ...
      const result = await Swal.fire({
        ...
        html: `Anda akan menghapus<br/>${namaPemasukan}`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        await Swal.fire({
          title: "Hapus data pemasukan berhasil!",
          icon: "success",
        }).then(() => {
          window.location.reload();
        });
      ...
      console.log(error);
      Swal.fire({
        title: "Hapus data pemasukan gagal!",
        text: `Gagal karena ${error.response.data.message}`,
        icon: "error",
      }).then(() => {
        // window.location.reload();
  ...
  allPemasukan.map((record) => {
    record.tgl = new Date(record.tgl);
  });

  const total = allPemasukan
